...
[Instructions:
The section sequence for this variant — one row per section, in emission order. `Guidance` is the content brief: what the section delivers, with approximate length/space — one-pagers are space-constrained.

Example (sales):]

| Section | Guidance |
|---------|----------|
| Header | Persona + value prop, one line |
| The problem | 2-3 sentences naming the pain |
| The solution | 2-3 sentences on the fix |
| Differentiators | 3 bullets |
| Use cases | 3 bullets |
| Proof | 1-2 customer references + one metric |
| CTA | The single next step |
...
- Whether the variant supports a secondary CTA (default: one primary — adding a second dilutes)
